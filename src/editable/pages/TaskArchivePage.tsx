...
const postStatus = (post: SitePost) => {
  const status = (post as unknown as { status?: unknown }).status
  return typeof status === 'string' ? status.toUpperCase() : ''
}

const mediaDistributionType = (post: SitePost) => {
  const content = getContent(post)
  const explicit = asText(content.type).toLowerCase().replace(/[\s_-]/g, '')
  const tags = Array.isArray(post.tags) ? post.tags.map((tag) => String(tag).toLowerCase().replace(/[\s_-]/g, '')) : []
  return explicit || tags.find((tag) => ['mediadistribution', 'pressrelease', 'newsmedia', 'publicrelation'].includes(tag)) || ''
}

const isRealMediaDistributionPost = (post: SitePost) => {
  if (postStatus(post) && postStatus(post) !== 'PUBLISHED') return false
  if (getPostTaskKey(post) === 'mediaDistribution') return true
  const type = mediaDistributionType(post)
  return ['mediadistribution', 'pressrelease', 'newsmedia', 'publicrelation'].includes(type)
}

const uniquePosts = (posts: SitePost[]) => Array.from(new Map(posts.map((post) => [post.id || post.slug || post.title, post])).values())

const localPagination = (page: number, limit: number, total: number): SiteFeedPagination => {
...
export async function EditableTaskArchiveRoute({
  ...
  let posts = scoped.posts
  let pagination = scoped.pagination

  // Some media-distribution route aliases are backed by feeds that ignore the task query.
  // Keep the archive on real API data by retrying against the unscoped feed before rendering empty.
  if (task === 'mediaDistribution' && !posts.length) {
    const feed = await fetchSiteFeed(1000, { fresh: true, category: category === 'all' ? undefined : category, timeoutMs: 5000 })
    const matching = uniquePosts((feed?.posts || []).filter((post) => {
      if (!isRealMediaDistributionPost(post)) return false
      if (category === 'all') return true
      return normalizeCategory(getCategory(post, '')) === category
    }))
    posts = matching.slice((page - 1) * limit, page * limit)
    pagination = localPagination(page, limit, matching.length)
  }
...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="editable-page-enter min-h-screen bg-white text-[#111]">
        <section className="border-b border-black bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-black/50">The distribution desk</p>
              <h1 className="mt-3 text-6xl font-black uppercase leading-none tracking-[-0.04em] sm:text-7xl lg:text-8xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md border-l-8 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-black/65">
              Real media distribution posts, public updates, and announcement cards organized for fast discovery.
            </p>
          </div>
        </section>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="editable-image-zoom group relative min-h-[34rem] overflow-hidden border-b border-black lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                ...
                <h2 className="mt-5 max-w-4xl text-4xl font-black uppercase leading-[0.98] tracking-[-0.045em] sm:text-6xl">{lead.title}</h2>
            ...
            <div className="grid">
              <div className="border-b border-black bg-[var(--slot4-accent)] p-6 text-black">
                ...
                <p className="mt-3 text-3xl font-black uppercase leading-tight">What the distribution desk is watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-black bg-white last:border-b-0">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-3 text-xl font-black uppercase leading-tight">{post.title}</h3>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-black bg-[#f7f7f2] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-black pb-4">
            <h2 className="text-4xl font-black uppercase tracking-[-0.04em] sm:text-5xl">Post cards</h2>
          ...
          {remaining.length ? (
            <div className="grid border-l border-t border-black md:grid-cols-2 xl:grid-cols-3 editable-stagger">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="editable-card-hover group border-b border-r border-black bg-white">
                  <div className="editable-image-zoom aspect-[16/10] overflow-hidden bg-black">
                  ...
                  <div className="p-5 sm:p-6">
                    ...
                    <h3 className="mt-4 text-2xl font-black uppercase leading-[1.05]">{post.title}</h3>
            ...
            <div className="border border-dashed border-black bg-white p-12 text-center">
              ...
              <h2 className="mt-4 text-3xl font-black uppercase">No real posts found</h2>
